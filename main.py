...
class Launcher(QMainWindow):
    ...
    def runmc(self, mcoptions, mcversion):
        ...
        thread = threading.Thread(target=call(
            get_minecraft_command(version=mcversion, minecraft_directory=minecraft_directory, options=mcoptions)))
        thread.start()
        thread.join()
        Launcher.show(self)
    ...
    def installmc(self, mcoptions, mcversion):
        ...
        thread = threading.Thread(
            target=install_minecraft_version(versionid=mcversion, minecraft_directory=minecraft_directory,
                                             callback=callback))
        thread.start()
        thread.join()
